...
for path in paths:
    with open(path, "r") as read_file:
        assets_strings.append("\tinline const std::string " + os.path.relpath(path, root_directory).replace(os.path.sep, "_").replace(".", "_").upper() + " {\"" + read_file.read().replace("\n", "\\n") + "\\0\"};\n")

with open(os.path.join(root_directory, "assets.hpp"), "w") as write_file:
    write_file.write("//GENERATED WITH assets_converter.py\n\n")
