...
for path in paths:
    with open(path, "r") as read_file:
        assets_strings.append("\tinline constexpr std::string_view " + os.path.relpath(path, root_directory).replace(os.path.sep, "_").replace(".", "_").upper() + " {\"" + ' '.join(read_file.read().replace("\n", "\\n").split()) + "\\0\"};\n")
